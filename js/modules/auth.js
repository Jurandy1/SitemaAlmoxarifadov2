...
async function initFirestoreListeners(renderDash, renderControls, renderModules) {
    ...
    // Água
    addListener(query(COLLECTIONS.aguaMov, orderBy("registradoEm", "desc"), limit(10000)), snap => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    // Gás
    addListener(query(COLLECTIONS.gasMov, orderBy("registradoEm", "desc"), limit(10000)), snap => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    // Materiais
    addListener(query(COLLECTIONS.materiais, orderBy("registradoEm", "desc"), limit(10000)), snap => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        setMateriais(data);
        scheduleRenders({ dash: true, modules: true }, renderDash, renderControls, renderModules);
    });
    ...
    // Assistência Social
    addListener(query(COLLECTIONS.cestaMov, orderBy("registradoEm", "desc"), limit(10000)), snap => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    addListener(query(COLLECTIONS.enxovalMov, orderBy("registradoEm", "desc"), limit(10000)), snap => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
