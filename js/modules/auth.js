...
import { initialAuthToken, auth } from "../firebase-config.js"; // ✅ auth agora vem do firebase-config.js
...
async function initAuthAndListeners(renderDashboardCallback, renderControlsCallback, renderUIModuleCallback) {
    if (!auth) { 
        console.error("⚠️ Auth não inicializado. Verifique firebase-config.js."); 
        return; 
    }

    if (DOM_ELEMENTS.connectionStatusEl) {
        DOM_ELEMENTS.connectionStatusEl.innerHTML = `
            <span class="h-3 w-3 bg-yellow-400 rounded-full animate-pulse"></span> 
        ...
        if (user) {
            ...
            if (DOM_ELEMENTS.connectionStatusEl)
                DOM_ELEMENTS.connectionStatusEl.innerHTML = `
                    ...
                    <span class="text-red-700">Desconectado</span>`;
        }
    });
    ...
    try {
        ...
        if (DOM_ELEMENTS.connectionStatusEl)
            DOM_ELEMENTS.connectionStatusEl.innerHTML = `
                <span class="h-3 w-3 bg-red-500 rounded-full"></span> 
                <span class="text-red-700">Erro Auth</span>`;
        showAlert('alert-agua', 
