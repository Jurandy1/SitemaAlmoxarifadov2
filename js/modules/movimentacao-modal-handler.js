...
export async function handleFinalMovimentacaoSubmit() {
    ...
    try {
        ...
        if (operacoesComErro > 0) {
            // Se ambas falharam ou apenas uma operação existia e falhou
            if (operacoesComErro === ( (qtdEntregue > 0 ? 1 : 0) + (qtdRetorno > 0 ? 1 : 0) ) ) {
                throw new Error(`Falha ao salvar ${tipoMovimentacao}. Verifique o console.`);
            } else {
                 // Se uma operação falhou mas a outra funcionou (caso de troca)
                 showAlert(alertId, `Movimentação parcialmente salva! ${msgSucesso.join('; ')}. Uma parte da ${tipoMovimentacao} falhou. Verifique o console.`, 'warning', 8000);
            }
        } else {
            // Sucesso completo
        ...
        // Limpa o formulário apenas se tudo deu certo (ou parcialmente certo)
        ...
        showAlert(alertId, `Erro ao salvar movimentação: ${error.message}`, 'error');
        ...
        // Por simplicidade, usamos um genérico
