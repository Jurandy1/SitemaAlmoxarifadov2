...
import { Timestamp, addDoc, updateDoc, doc, serverTimestamp } from "firebase/firestore";
import { getMateriais, getUserRole } from "../utils/cache.js"; // Adicionado getUserRole
// CORREÇÃO: DOM_ELEMENTOS -> DOM_ELEMENTS
import { DOM_ELEMENTS, showAlert, filterTable, switchSubTabView, escapeHTML } from "../utils/dom-helpers.js";
...
export function renderMateriaisStatus() {
    ...
    if (batchSelect) {
        ...
        if (batchSelect.querySelector(`option[value="${escapeHTML(atual)}"]`)) {
...
function renderMaterialSubTable(tableBody, data, status) {
    ...
    data.forEach(m => {
        ...
        const idSafe = escapeHTML(m.id);
        const fileURLSafe = escapeHTML(m.fileURL);
        const unidadeNomeSafe = escapeHTML(m.unidadeNome || '');
        const unidadeDisplaySafe = escapeHTML(unidadeDisplay);
        const tipoMaterialSafe = escapeHTML(m.tipoMaterial || '');
        const itensSafe = escapeHTML(m.itens || '');
        const responsavelLancamentoSafe = escapeHTML(responsavelLancamento);
        const separadorSafe = escapeHTML(separador);
        const downloadBtn = hasFile 
            ? `<button class="btn-icon btn-download-pedido text-blue-600 hover:text-blue-800" data-id="${idSafe}" data-url="${fileURLSafe}" title="Baixar Pedido" aria-label="Baixar Pedido"><i data-lucide="download" class="w-4 h-4"></i></button>`
        ...
        const removeBtn = isAdmin
            ? `<button class="btn-icon btn-remove text-red-600 hover:text-red-800" data-id="${idSafe}" data-type="materiais" data-details="${unidadeNomeSafe} - ${escapeHTML(status)}" title="Remover Requisição" aria-label="Remover"><i data-lucide="trash-2" class="w-4 h-4"></i></button>`
            : '';
        ...
        if (status === 'requisitado') {
            const startSeparacaoBtn = canEditFlow
                ? `<button class="btn-icon btn-start-separacao text-green-600 hover:text-green-800" data-id="${idSafe}" title="Informar Separador e Iniciar" aria-label="Iniciar separação"><i data-lucide="play-circle" class="w-4 h-4"></i></button>`
            ...
            rowContent = `<td>${unidadeDisplaySafe}</td>` +
                `<td class="capitalize">${tipoMaterialSafe}</td>` +
                `<td class="whitespace-nowrap">${dataRequisicaoFormatada}</td>` +
                `<td>${responsavelLancamentoSafe}</td>` +
            ...
            const prontaRetiradaBtn = canEditFlow
                ? `<button class="btn-icon btn-retirada text-teal-600 hover:text-teal-800" data-id="${idSafe}" title="Marcar como pronto para entrega" aria-label="Pronto para entrega"><i data-lucide="package-check" class="w-4 h-4"></i></button>`
            ...
            rowContent = `<td>${unidadeDisplaySafe} ${slaBadge}</td>` +
                `<td class="capitalize">${tipoMaterialSafe}</td>` +
                `<td>${separadorSafe}</td>` +
            ...
            const finalizarEntregaBtn = canFinalize
                ? `<button class="btn-icon btn-entregue text-blue-600 hover:text-blue-800" data-id="${idSafe}" title="Finalizar entrega e registrar responsáveis" aria-label="Finalizar entrega"><i data-lucide="check-circle" class="w-4 h-4"></i></button>`
            ...
            const respUnidadeSafe = escapeHTML(respUnidade);
            const respAlmoxSafe = escapeHTML(respAlmox);
        ...
        if (m.itens) {
            html += `<tr class="obs-row ${status === 'entregue' ? 'opacity-60' : ''} border-b border-slate-200">` +
                ...
                `<td colspan="${status === 'entregue' ? '7' : '5'}" class="pt-0 pb-1 px-6 text-xs text-slate-500 whitespace-pre-wrap italic">Obs: ${itensSafe}</td>` +
                `</tr>`;
        }
    });
...
async function handleDownloadPedido(materialId, fileURL) {
    ...
    // Abre o link ANTES do await para não perder o contexto de clique do usuário
    // (navegadores bloqueiam window.open após chamadas assíncronas)
