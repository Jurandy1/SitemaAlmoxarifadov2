...
/** Siglas do organograma SEMCAS → nome cadastrado (resolvido depois via resolveRegisteredUnit). */
const UNIT_ACRONYM_HINTS = [
  { re: /\bSPSB\b/i, hint: 'SPSB' },
  { re: /\bSPSE\b.*M[EÉ]DIA/i, hint: 'SPSE - MEDIA COMPLEXIDADE' },
  { re: /\bSPSE\b/i, hint: 'SPSE - MEDIA COMPLEXIDADE' },
  { re: /\bSGBSTR\b/i, hint: 'TRANSFERÊNCIA DE RENDA-SGBSTR' },
  { re: /\bSEOF[CÇ]\b/i, hint: 'SUPERINTENDÊNCIA DE EXECUÇÃO ORÇAMENTÁRIA' },
  { re: /\bGSUAS\b/i, hint: 'SUPERINTENDÊNCIA DE GESTÃO DO SUAS' },
  { re: /\bSAPS\b/i, hint: 'SECRETARIA ADJUNTA DE PROTEÇÃO SOCIAL' },
];

function stripUnitTrailingNoise(name) {
  return String(name || '')
    .replace(/\s+\d{1,2}\s*[\/\.]\s*\d{1,2}\s*[\/\.]\s*\d{2,4}\s*$/i, '')
    .replace(/\s+\d{1,2}[\/\.]\d{1,2}[\/\.]?\d{0,4}.*/i, '')
...
function extractUnitFromCategoryLine(text) {
  const raw = String(text || '').trim();
  if (!raw || isSkipLine(raw)) return null;

  for (const { re, hint } of UNIT_ACRONYM_HINTS) {
    if (re.test(raw)) return hint;
  }

  const patterns = [
    /nome\s+da\s+unid(?:ade|e)?\s*[:;]?\s*(.+)/i,
    /CRAS\s+TERRIT[OÓ]RIO\s*\d+\s*[-–]?\s*[A-ZÁÉÍÓÚÃÕÂÊÔÇ\s]+/i,
    /CRAS\s+[A-ZÁÉÍÓÚÃÕÂÊÔÇ][A-ZÁÉÍÓÚÃÕÂÊÔÇ\s]{2,}/i,
    /CREAS\s+[A-ZÁÉÍÓÚÃÕÂÊÔÇ][A-ZÁÉÍÓÚÃÕÂÊÔÇ\s\/]{2,}/i,
    /\bCT\s+[A-ZÁÉÍÓÚÃÕÂÊÔÇ][A-ZÁÉÍÓÚÃÕÂÊÔÇ\s\/]{2,}/i,
    /CENTRO\s*POP\s+[A-ZÁÉÍÓÚÃÕÂÊÔÇ][A-ZÁÉÍÓÚÃÕÂÊÔÇ\s\/]*/i,
    /SUPERINTEND[EÊ]NCIA[^\-–\d]{4,}/i,
    /COORDENA[CÇ][AÃ]O[^\-–\d]{4,}/i,
    /DIRETORIA\s+T[EÉ]CNICA[^\-–\d]{4,}/i,
    /CONSELHO\s+TUTELAR/i,
    /TRANSFER[EÊ]NCIA\s+DE\s+RENDA/i,
  ];

  for (const re of patterns) {
    const m = raw.match(re);
    if (m) {
      const piece = stripUnitTrailingNoise(m[1] || m[0]);
      if (piece && piece.length >= 3) return piece;
    }
  }
  return null;
}
...
/**
 * Busca unidade no título (topo), colunas de cabeçalho e títulos de categoria/itens.
 * Retorna { raw, source } ou null.
 */
function inferUnitFromSheet(rows, fileName = '') {
  const candidates = [];
  const push = (raw, score, source) => {
    const clean = stripUnitTrailingNoise(raw);
    if (!clean || clean.length < 2 || isSkipLine(clean)) return;
    if (/^(material|materiais|unidade|quantidade|qtd|especifica)/i.test(clean)) return;
    candidates.push({ raw: clean, score, source });
  };
  ...
  for (let ri = 0; ri < scanLimit; ri++) {
    ...
    for (let ci = 0; ci < r.length; ci++) {
      ...
      if (lo.includes('nome da unid')) {
        const m = s.match(/nome\s+da\s+unid(?:ade|e)?\s*[:;]?\s*(.+)/i);
        if (m) push(m[1], 100, 'titulo-nome-da-unidade');
        continue;
      }

      if (lo === 'material' || lo === 'materiais') {
        const next = n(r[ci + 1]);
        if (next && next.toLowerCase() !== 'unidade' && next.length < 80) {
          push(next, 85, 'coluna-cabecalho');
        }
        continue;
      }

      if (/^(CT|CRAS|CREAS|ABRIGO|SEDE|PROCAD|CENTRO\s*POP|AEPETI|ASTEC|CMDCA|CMDCMA|CMAS|COGETEP|PCDIF|IGAS|SPSB|SPSE|SGBSTR|CONSELHO\s+TUTELAR)/i.test(s) && !lo.includes('nome da unid')) {
        push(s, ri < 8 ? 90 : 70, ri < 8 ? 'titulo-topo' : 'linha-superior');
      }

      for (const { re, hint } of UNIT_ACRONYM_HINTS) {
        if (re.test(s)) push(hint, 88, 'sigla');
      }
    }
  }
  ...
  for (const r of rows) {
    const f = n(r[0]);
    if (!f) continue;
    if (isCategory(r) || /^\d+\s*[-–—.]\s*.+/.test(f)) {
      const embedded = extractUnitFromCategoryLine(f);
      if (embedded) push(embedded, 55, 'titulo-categoria');
    }
    if (isHeader(r)) {
      const hdrUnit = extractUnitFromCategoryLine(f) || extractUnitFromCategoryLine(n(r[1]));
      if (hdrUnit) push(hdrUnit, 60, 'cabecalho-secao');
    }
  }

  if (fileName) {
    const fm = fileName.match(/(CRAS|CREAS|CENTRO\s*POP|CT|PROCAD|AEPETI|SPSB|SPSE|SGBSTR|CONSELHO\s*TUTELAR)[a-zA-ZáéíóúãõâêôçÁÉÍÓÚÃÕÂÊÔÇ\s_0-9\-–/]+/i);
    if (fm) push(fm[0].replace(/[-_]/g, ' ').trim(), 50, 'nome-arquivo');
  }
  ...
  return { raw: best.raw, source: best.source, all: candidates.slice(0, 5) };
}

function extractUnit(rows, fileName = '') {
  const inferred = inferUnitFromSheet(rows, fileName);
  if (inferred?.raw) return inferred.raw;
  return 'Unidade';
}

function parsePadrao(rows, fileName = ''){
  const un=extractUnit(rows, fileName);
...
function parseSheet(rows, fileName = ''){
  ...
  const fmt=detectFormat(rows);
  if(fmt==='abrigo')return parseAbrigo(rows);
  return parsePadrao(rows, fileName);
}
...
function handleFile(e){
  ...
  reader.onload=async (ev)=>{try{
    ...
    tmpParsed=parseSheet(rows, file.name);
    ...
    const inferred = inferUnitFromSheet(rows, file.name);
    if (inferred) tmpParsed._unitDetectSource = inferred.source;

    if (tmpParsed.unitName === 'Unidade' || tmpParsed.unitName === 'Desconhecida') {
      if (inferred?.raw) {
        tmpParsed.unitName = inferred.raw;
      } else {
        const m = file.name.match(/(CRAS|CREAS|CENTRO POP|CT|PROCAD|AEPETI|ASTEC|ILPI|CAT|POP RUA|SPSB|SPSE|SGBSTR)[a-z\s_0-9-ãõáéíóú]+/i);
        if(m) tmpParsed.unitName = m[0].replace(/[-_]/g, ' ').trim();
      }
    }
    ...
    const rawFromSheet = String(tmpParsed.unitName || '').trim();
    const detectSrc = tmpParsed._unitDetectSource;
    const srcTag = detectSrc
      ? `<span style="font-size:10px;color:#64748b;margin-left:6px">(detectado: ${esc(detectSrc)})</span>`
      : '';
    const matchedUnit = resolveRegisteredUnit(rawFromSheet);
    const isUnknown = !matchedUnit;
    const unitDisplayTag = matchedUnit
      ? `<b style="color:#166534">Sugestão: ${esc(matchedUnit)}</b> <span style="font-size:11px;color:var(--muted)">— confirme ou escolha abaixo</span>${srcTag}`
      : `<span style="color:#92400e">Na planilha: <b>${esc(rawFromSheet || '—')}</b>${srcTag} · <b>escolha a unidade correta abaixo</b></span>`;
    ...
    if (matchedUnit) {
      for (let i = 0; i < uSel.options.length; i++) {
        const ov = uSel.options[i].value;
        if (ov === matchedUnit || resolveSelectedUnit(ov) === matchedUnit) {
    ...
    const needsPick = !found || !isRegisteredUnit(uSel.value);
    if (needsPick) {
      ...
      <div style="display:flex;align-items:flex-start;gap:7px;margin-bottom:8px">
        ...
        <div>
          <b style="color:#92400e;font-size:12px;display:block;margin-bottom:2px">
            Unidade da planilha não foi reconhecida no cadastro.
          </b>
          <span style="font-size:11px;color:#78350f">
            Escolha obrigatoriamente uma unidade da lista abaixo. O sistema não cria unidade nova pela planilha.
          </span>
        </div>
      </div>
...
function ck(){
  const uSel = document.getElementById('rU');
  const hasUnit = isRegisteredUnit(uSel?.value || '');
  const canReg = !!tmpParsed && hasUnit;
  const bReg = document.getElementById('bR');
  if (bReg) bReg.disabled = !canReg;
  const bPreview = document.getElementById('bPreview');
  if (bPreview) bPreview.disabled = !canReg;
  if (uSel && hasUnit) {
    uSel.style.borderColor = '#10b981';
    uSel.style.boxShadow = '0 0 0 3px rgba(16,185,129,.12)';
    const warnEl = document.getElementById('rUWarn');
...
function buildPreviewReqObject() {
  ...
  const uSel=document.getElementById('rU');
  const unidade = resolveSelectedUnit(uSel?.value || '') || '';
...
// ═══════════════════════════════════════════════════════════════════
// DETECÇÃO DE PISTAS — analisa unidade/obs para sugerir opção padrão
// ═══════════════════════════════════════════════════════════════════
function detectItemHints(item) {
  const unid = String(item.unidade || '').toUpperCase().trim();
  const obs  = String(item.obs || '').toUpperCase();
  const all  = unid + ' ' + obs;
  return {
    isLiquid: /\b(L|LT|LITRO|LITROS|ML|MILILITRO|FR|FRASCO|GARRAFA)\b/.test(all) || /\bLIQUID/i.test(all),
    isPowder: /\b(KG|KGS|G|GR|GRAMA|GRAMAS|PCT|PACOTE|PACOTES|SC|SACO|SACOS|CX|CAIXA|CAIXAS)\b/.test(unid) || /\bEM\s*PO\b|\bP[OÓ]\b/i.test(all),
    isBar:    /\b(BARRA|BARRAS|PEDRA|PEDRAS|TABLETE|TABLETES|UND|UN|UNID|UNIDADE)\b/.test(unid) && /SABAO|SABONETE/i.test(all),
    isCan:    /\b(LT|LATA|LATAS|LT\.)\b/.test(unid),
    unidade: unid,
    obs
  };
}
  ...
  parsed.categories.forEach((cat, catIdx) => {
    cat.items.forEach((item, itemIdx) => {
      ...
      if (isCopo && !hasAgua && !hasCafe) {
        const h = detectItemHints(item);
        let sugg = null;
        // Copo pequeno (50ml) geralmente é café; 180-200ml é água
        if (/\b(180|200|250)\s*ML\b/i.test(h.unidade + ' ' + h.obs)) sugg = 'COPO DESCARTÁVEL PARA ÁGUA';
        else if (/\b(50|80)\s*ML\b/i.test(h.unidade + ' ' + h.obs)) sugg = 'COPO DESCARTÁVEL PARA CAFÉ';
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: ['COPO DESCARTÁVEL PARA ÁGUA', 'COPO DESCARTÁVEL PARA CAFÉ'],
          suggested: sugg,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Copo genérico — escolha se é para água ou café'
        });
        return;
      }
      ...
      // ── SABÃO ── (em pó, em barra, em pedra, líquido) — ESCOLHA OBRIGATÓRIA
      ...
      if ((/^\s*SABAO\b/i.test(matNorm) || /^\s*SABOES\b/i.test(matNorm)) && !sabaoType && !hasAgua) {
        const h = detectItemHints(item);
        const opts = ['SABÃO EM PÓ', 'SABÃO EM BARRA', 'SABÃO LÍQUIDO'];
        let sugg = null;
        if (h.isPowder) sugg = 'SABÃO EM PÓ';
        else if (h.isLiquid) sugg = 'SABÃO LÍQUIDO';
        else if (h.isBar)    sugg = 'SABÃO EM BARRA';
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: opts,
          suggested: sugg,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Sabão genérico — escolha obrigatoriamente UM tipo'
        });
        return;
      }
      
      // ── LEITE ── (em pó, líquido, condensado) — ESCOLHA OBRIGATÓRIA
      const isLeite = /^\s*LEITE\s*$/i.test(matNorm);
      if (isLeite) {
        const h = detectItemHints(item);
        const opts = ['LEITE EM PÓ', 'LEITE LÍQUIDO', 'LEITE CONDENSADO'];
        let sugg = null;
        if (h.isPowder) sugg = 'LEITE EM PÓ';            // PCT, KG, SACO → pó
        else if (h.isLiquid) sugg = 'LEITE LÍQUIDO';      // L, ML, LITRO → líquido
        else if (h.isCan)    sugg = 'LEITE CONDENSADO';   // LATA → condensado
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: opts,
          suggested: sugg,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Leite genérico — escolha obrigatoriamente UM tipo'
        });
        return;
      }
      
      // ── COLA ── (branca, bastão, quente, isopor) — ESCOLHA OBRIGATÓRIA
      const isColaGeneric = /^\s*COLA\s*$/i.test(matNorm) || /^\s*COLAS\s*$/i.test(matNorm);
      if (isColaGeneric) {
        const h = detectItemHints(item);
        const opts = ['COLA BRANCA', 'COLA BASTÃO', 'COLA QUENTE'];
        let sugg = null;
        if (h.isLiquid) sugg = 'COLA BRANCA';
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: opts,
          suggested: sugg,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Cola genérica — escolha obrigatoriamente UM tipo'
        });
        return;
      }
      
      // ── PAPEL ── (A4, toalha, higiênico) — ESCOLHA OBRIGATÓRIA
      const isPapelGeneric = /^\s*PAPEL\s*$/i.test(matNorm) || /^\s*PAPEIS\s*$/i.test(matNorm);
      if (isPapelGeneric) {
        const h = detectItemHints(item);
        const opts = ['RESMA DE PAPEL A4', 'PAPEL TOALHA', 'PAPEL HIGIÊNICO'];
        let sugg = null;
        if (/\b(RESMA|PCT|PACOTE)\b/i.test(h.unidade)) sugg = 'RESMA DE PAPEL A4';
        else if (/\b(RL|ROLO|ROLOS)\b/i.test(h.unidade)) sugg = 'PAPEL HIGIÊNICO';
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: opts,
          suggested: sugg,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Papel genérico — escolha obrigatoriamente UM tipo'
        });
        return;
      }
      ...
      // ── CLIPS ── OBRIGATÓRIO especificar tamanho — ESCOLHA OBRIGATÓRIA
      const isClipsGeneric = /^\s*CLIPE?S?\s*$/i.test(matNorm) || /^\s*CLIPE?S?\s+PARA\s+PAPEL\s*$/i.test(matNorm);
      if (isClipsGeneric) {
        const h = detectItemHints(item);
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: ['CLIPS Nº 2/0', 'CLIPS Nº 3/0', 'CLIPS Nº 4/0', 'CLIPS Nº 6/0', 'CLIPS Nº 8/0'],
          suggested: null,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Clips genérico — escolha obrigatoriamente UM tamanho'
        });
        return;
      }
      ...
      // ── PASTA ── só diz PASTA — ESCOLHA OBRIGATÓRIA
      const isPastaGeneric = /^\s*PASTAS?\s*$/i.test(matNorm);
      if (isPastaGeneric) {
        const h = detectItemHints(item);
        issues.push({
          type: 'choose', catIdx, itemIdx, original: rawMat, item,
          options: ['PASTA AZ', 'PASTA SANFONADA', 'PASTA SUSPENSA', 'PASTA POLIONDA'],
          suggested: null,
          hint: h.unidade ? `Unidade na planilha: "${h.unidade}"` : '',
          reason: 'Pasta genérica — escolha obrigatoriamente UM tipo'
        });
        return;
      }
...
async function registrar(){
  ...
  // ─── VALIDAÇÃO: UNIDADE OBRIGATÓRIA (somente cadastradas) ───
  const uSel = document.getElementById('rU');
  const selectedUnit = String(uSel?.value || '').trim();
  if (!selectedUnit) {
    toast('⚠️ Selecione a unidade cadastrada no campo "Unidade" antes de registrar.', 'red');
    uSel?.focus?.();
  ...
  if (!isRegisteredUnit(selectedUnit)) {
    toast('⚠️ A unidade selecionada não está cadastrada. Use Gestão de Unidades para cadastrar.', 'red');
    return;
  }
...
async function doRegistrar(finalUnit){
  const uSel=document.getElementById('rU');
  const unidade = resolveSelectedUnit(finalUnit || uSel?.value || '');
  if (!unidade) {
    toast('⚠️ Unidade inválida. Selecione uma unidade cadastrada.', 'red');
    return;
  }
  const itemsMap={};tmpParsed.categories.forEach(c=>c.items.forEach(it=>{itemsMap[it.id]={...it}}));
  const parsedSafe = stripVolatileParsed(tmpParsed);
...
function addReqToHistDB(req) {
  ...
  const unitName = resolveRegisteredUnit(req.unidade) || resolveRegisteredUnit(normalizeUnit(req.unidade));
  if (!unitName) {
    console.warn('Requisição ignorada no histórico: unidade não cadastrada', req.unidade);
    return null;
  }
...
function buildHistEntryFromParsed(parsed,fin){
  ...
  const unitName = resolveRegisteredUnit(parsed.unitName) || resolveRegisteredUnit(normalizeUnit(parsed.unitName));
...
const UNIT_MAP=[
  ...
  [/^cras\s+vinhais?\b/i,                                       'Cras Vinhas'],
  [/cras\s*territ[oó]rio\s*2.*fati/i,                          'Cras Bairro De Fatima'],
  [/coordena[cç][aã]o\s+de\s+transporte/i,                     'Diretoria Técnica De Transporte'],
  [/superintend[eê]ncia\s+(de\s+administra[cç][aã]o|administrativa)/i, 'SuperintendêNcia De AdministraçãO'],
  [/^cras\s+territ[oó]rio\s*18/i,                               'Cras Janaina'],
  [/^spsb$/i,                                                   'SuperintendêNcia De ProteçãO Social BáSica'],
  [/\bspsb\b/i,                                                 'SuperintendêNcia De ProteçãO Social BáSica'],
  [/spse.*m[eé]dia\s*complex/i,                                 'SuperintendêNcia De ProteçãO Social Especial De MéDia Complexidade'],
  [/transfer[eê]ncia\s+de\s+renda/i,                           'Diretoria Técnica De Cadastro úNico E TransferêNcia De Renda'],
];
function getRegisteredUnitsList() {
  return (getUnidades() || []).filter((u) => (u?.atendeMateriais ?? true) !== false && !u?.duplicataDe);
}

function getRegisteredUnitName(u) {
  return String(u?.nome || u?.unidadeNome || '').trim();
}

function parseUnitSelectValue(raw) {
  const s = String(raw || '').trim();
  const m = s.match(/^(.+)\|(CREAS|CRAS|CT|SEDE|POP|ABRIGO|OUTROS)$/i);
  if (!m) return { nome: s, tipo: null };
  return { nome: m[1].trim(), tipo: m[2].toUpperCase() };
}

function resolveSelectedUnit(selectValue) {
  const { nome, tipo } = parseUnitSelectValue(selectValue);
  if (!nome) return null;
  const list = getRegisteredUnitsList();
  if (tipo) {
    const hit = list.find((u) =>
      registeredUnitKey(getRegisteredUnitName(u)) === registeredUnitKey(nome) &&
      String(u.tipo || '').toUpperCase() === tipo
    );
    if (hit) return getRegisteredUnitName(hit);
  }
  // Escolha manual na lista: nome cadastrado exato, sem alias/fuzzy
  const exact = list.find((u) => registeredUnitKey(getRegisteredUnitName(u)) === registeredUnitKey(nome));
  if (exact) return getRegisteredUnitName(exact);
  return resolveRegisteredUnit(nome);
}

function registeredUnitKey(name) {
  return rmAcc(String(name || '')).toUpperCase().replace(/\s+/g, ' ').trim();
}

function isRegisteredUnit(name) {
  const { nome, tipo } = parseUnitSelectValue(name);
  const k = registeredUnitKey(nome);
  if (!k) return false;
  const list = getRegisteredUnitsList();
  if (tipo) {
    return list.some((u) => registeredUnitKey(getRegisteredUnitName(u)) === k && String(u.tipo || '').toUpperCase() === tipo);
  }
  return list.some((u) => registeredUnitKey(getRegisteredUnitName(u)) === k);
}

/** Retorna o nome oficial cadastrado ou null — nunca inventa unidade nova. */
function resolveByCadastroSigla(text) {
  const k = rmAcc(String(text || '')).toUpperCase();
  if (!k) return null;
  for (const u of getRegisteredUnitsList()) {
    const sigla = rmAcc(u?.sigla || '').toUpperCase().trim();
    if (!sigla || sigla.length < 2) continue;
    const esc = sigla.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    if (new RegExp(`\\b${esc}\\b`).test(k)) return getRegisteredUnitName(u);
  }
  return null;
}

function resolveAcronymHint(text) {
  const s = String(text || '');
  for (const { re, hint } of UNIT_ACRONYM_HINTS) {
    if (!re.test(s) && !re.test(rmAcc(s))) continue;
    const aliasKey = rmAcc(hint).toUpperCase().replace(/\s+/g, ' ').trim();
    if (HIST_ALIASES[aliasKey]) {
      const t = HIST_ALIASES[aliasKey];
      if (isRegisteredUnit(t)) return t;
    }
    const exact = getRegisteredUnitsList().find((u) => registeredUnitKey(getRegisteredUnitName(u)) === aliasKey);
    if (exact) return getRegisteredUnitName(exact);
    for (const [re2, canon] of UNIT_MAP) {
      if (re2.test(hint) && isRegisteredUnit(canon)) return canon;
    }
  }
  return null;
}

function resolveRegisteredUnit(raw) {
  if (!raw) return null;
  const s = String(raw).trim().replace(/\s+/g, ' ');
  const k = rmAcc(s).toUpperCase();

  const bySigla = resolveByCadastroSigla(s);
  if (bySigla) return bySigla;

  const byAcronym = resolveAcronymHint(s);
  if (byAcronym) return byAcronym;

  if (/^(fornecimento|data:|unidade de acolhimento$|separado|entregue|recebido)/i.test(s)) return null;

  const unidades = getRegisteredUnitsList();
  ...
  // Nome exato no cadastro vence alias e fuzzy (ex.: "Cras Estiva" ≠ "Cras Estiva Sub Prefeitura")
  const exactList = unidades.filter((u) => norm(getRegisteredUnitName(u)) === normRaw);
  if (exactList.length === 1) return getRegisteredUnitName(exactList[0]);
  if (exactList.length > 1) {
    const hintTipo = /\bCREAS\b/i.test(s) ? 'CREAS'
      : /\bCRAS\b/i.test(s) ? 'CRAS'
      : /\bCT\b/i.test(s) ? 'CT'
      : null;
    if (hintTipo) {
      const picked = exactList.find((u) => String(u.tipo || '').toUpperCase() === hintTipo);
      if (picked) return getRegisteredUnitName(picked);
    }
    return null;
  }

  if (HIST_ALIASES[k]) {
    const aliasTarget = HIST_ALIASES[k];
    return isRegisteredUnit(aliasTarget) ? aliasTarget : null;
  }
  for (const [re, canon] of UNIT_MAP) {
    if (re.test(s) || re.test(rmAcc(s))) {
      return isRegisteredUnit(canon) ? canon : null;
    }
  }

  const rawIsCreas = /^CREAS\b/.test(normRaw) || /\bCREAS\b/.test(normRaw);
  const rawIsCras = /^CRAS\b/.test(normRaw);

  const prefixHits = unidades.filter((u) => {
    const nu = norm(getRegisteredUnitName(u));
    if (rawIsCreas && /^CRAS\b/.test(nu)) return false;
    if (rawIsCras && /^CREAS\b/.test(nu)) return false;
    return nu.length >= 4 && (normRaw.startsWith(nu) || nu.startsWith(normRaw));
  });
  if (prefixHits.length) {
    const rawExtends = prefixHits
      .filter((u) => {
        const nu = norm(getRegisteredUnitName(u));
        return normRaw.startsWith(nu) && normRaw.length > nu.length;
      })
      .sort((a, b) => norm(getRegisteredUnitName(b)).length - norm(getRegisteredUnitName(a)).length);
    if (rawExtends.length) return getRegisteredUnitName(rawExtends[0]);

    const unitExtends = prefixHits
      .filter((u) => {
        const nu = norm(getRegisteredUnitName(u));
        return nu.startsWith(normRaw) && nu.length > normRaw.length;
      })
      .sort((a, b) => norm(getRegisteredUnitName(a)).length - norm(getRegisteredUnitName(b)).length);
    if (unitExtends.length) return getRegisteredUnitName(unitExtends[0]);
  }

  const byLen = [...unidades].sort((a, b) => {
    const la = norm(getRegisteredUnitName(a)).length;
    const lb = norm(getRegisteredUnitName(b)).length;
    return lb - la;
  });
  ...
  if (contains) return getRegisteredUnitName(contains);

  return null;
}

function normalizeUnit(raw){
  return resolveRegisteredUnit(raw) || 'Desconhecida';
}
...
function extractUnitFromRows(rows, fileName = ''){
  const inferred = inferUnitFromSheet(rows, fileName);
  if (inferred?.raw) return stripUnitTrailingNoise(inferred.raw);
...
function parseMultiSheetWb(wb,fileName,fy){
  ...
  for(const sn of wb.SheetNames){
    const rows=getSafeRows(wb.Sheets[sn]);
    const rawUnit=extractUnitFromRows(rows, fileName)||sn;
...
function handleHistFiles(e){
  ...
  if(!isRegisteredUnit(selectedUnit)){toast('A unidade selecionada não está cadastrada.','red');return;}
  const fy=new Date().getFullYear();
    ...
    reader.onload=async (ev)=>{
      try{
        ...
        if(entry&&entry.units&&entry.units.length){
          ...
          entry.units.forEach(u => {
            const official = resolveRegisteredUnit(selectedUnit);
            if (!official) return;
            u.unitName = official;
            u.rawUnit = u.rawUnit || selectedUnit;
          });
          entry.units = entry.units.filter((u) => u.unitName && isRegisteredUnit(u.unitName));
...
function populateUnidadesSelect() {
  ...
  const unidades = (getUnidades() || [])
    .filter((u) => (u?.atendeMateriais ?? true) === true && !u?.duplicataDe)
    .filter((u) => {
      if (!q) return true;
      const nome = String(u?.nome || u?.unidadeNome || "").trim();
  ...
  for (const u of unidades) {
    const nome = String(u?.nome || u?.unidadeNome || "").trim();
    if (!nome) continue;
    nameTotals.set(nome, (nameTotals.get(nome) || 0) + 1);
  }
  for (const u of unidades) {
    const nome = String(u?.nome || u?.unidadeNome || "").trim();
    if (!nome) continue;
    ...
    if (tipo === "SEMCAS") tipo = "SEDE";
    if (!groups.has(tipo)) groups.set(tipo, []);
    groups.get(tipo).push({
      nome,
      sigla: String(u?.sigla || "").trim(),
      tipo,
      dup: (nameTotals.get(nome) || 0) > 1,
    });
  }
  ...
  for (const tipo of tiposOrdenados) {
    ...
    for (const { nome, sigla, tipo, dup } of arr) {
      const opt = document.createElement("option");
      opt.value = dup ? `${nome}|${tipo}` : nome;
      if (sigla) opt.textContent = dup ? `${nome} — ${tipo}  [${sigla}]` : `${nome}  [${sigla}]`;
      else opt.textContent = dup ? `${nome} — ${tipo}` : nome;
      og.appendChild(opt);
    }
    sel.appendChild(og);
  }
...
function _suggestUnits(raw, limit = 5) {
  ...
  const scoreUnit = (u) => {
    ...
    let score = 0;
    if (rawNorm.startsWith(hay0) && hay0.length < rawNorm.length) score += 55;
    else if (hay0.startsWith(rawNorm) && hay0.length > rawNorm.length) score += 25;
    else if (hay0.includes(rawNorm) || rawNorm.includes(hay0)) score += 40;
  ...
  return units
    ...
    .sort((a, b) => b.score - a.score || a.name.length - b.name.length || a.name.localeCompare(b.name, "pt-BR"))
    .slice(0, limit)
    .map((x) => x.name);
}
