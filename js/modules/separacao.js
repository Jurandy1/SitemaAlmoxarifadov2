import { setDoc, deleteDoc, doc, writeBatch, getDocs, query, orderBy, limit, startAfter, serverTimestamp, Timestamp } from "firebase/firestore";
...
const DataStore = {
  ...
  async del(key) {
    if (!isReady()) throw new Error("Não autenticado");
    if (key === "hist_v4") {
      const snap = await getDocs(query(COLLECTIONS.semcasHistDB, limit(1000)));
      const docs = snap.docs;
      for (let i = 0; i < docs.length; i += 450) {
        const b = writeBatch(COLLECTIONS.semcasHistDB.firestore);
        docs.slice(i, i + 450).forEach((d) => b.delete(d.ref));
        await b.commit();
      }
    }
  },
...
function previewReq() {
  ...
  document.getElementById('fichaTitle').textContent='👁️ Pré-visualização: '+req.unidade;
  ...
  document.getElementById('fichaBody').innerHTML=buildFichaHTML(req, true);
...
function buildFichaHTML(r,isPrint){
  ...
  d.categories.forEach(cat=>{
    ...
    catItems.forEach((m,i)=>{
      ...
      if(isPrint){const qaDisplay=m.qtdAtendida?esc(m.qtdAtendida):'<span style="display:inline-block;width:90%;border-bottom:1px dotted #94a3b8;min-height:14px">&nbsp;</span>';h+=`<td class="col-ate" style="font-weight:700">${qaDisplay}</td><td class="col-status"><span class="${bc2(m.status)}" style="cursor:default">${sl(m.status)}</span></td><td class="col-obs" style="font-size:10px;color:#475569">${esc(m.obs||'')}</td>`}
      else{h+=`<td class="col-ate" style="padding:2px 4px"><input class="ficha-input-qty${m.status==='sem_estoque'?' no-stock':''}" value="${esc(m.qtdAtendida)}" placeholder="—"></td><td class="col-status" style="padding:2px"><span class="${bc2(m.status)}">${sl(m.status)}</span></td><td class="col-obs" style="padding:2px 4px"><input class="ficha-input-obs" value="${esc(m.obs)}" placeholder="Obs..."></td>`}
      h+=`</tr>`;
    });
    h+=`</tbody></table>`;
  });
...
async function clearHistDB(){
  ...
  // Apaga visualmente na hora para feedback rápido
  const items = [...HIST_DB];
  HIST_DB=[]; 
  invalidateAggCache();
  renderRelatorio();
  
  // Apaga do Firestore em lote
  let deletedCount = 0;
  try {
    const db = COLLECTIONS.semcasHistDB.firestore;
    for (let i = 0; i < items.length; i += 450) {
      const b = writeBatch(db);
      items.slice(i, i + 450).forEach((e) => {
        b.delete(doc(COLLECTIONS.semcasHistDB, e.id));
      });
      await b.commit();
      deletedCount += items.slice(i, i + 450).length;
    }
...
function removeHistEntry(id) {
  if (confirm("Tem certeza que deseja excluir esta planilha do banco de dados permanentemente?")) {
    HIST_DB = HIST_DB.filter(e => e.id !== id);
    
    // Also remove from Firestore to ensure it's permanently deleted
    try {
      deleteDoc(doc(COLLECTIONS.semcasHistDB, id)).then(() => {
        console.log("Documento deletado do Firestore com sucesso:", id);
      }).catch(err => {
        console.error("Erro ao deletar documento do Firestore:", err);
      });
    } catch (e) {
      console.error("Erro ao chamar deleteDoc:", e);
    }
...
function renderUnificar() {
  ...
  if (mergePanel) {
    if (UNIF_SEL.length >= 2) {
      ...
      UNIF_SEL.forEach(u => {
        mp += '<span style="background:#dbeafe;color:#1e40af;padding:4px 10px;border-radius:8px;font-size:11px;font-weight:700;display:inline-flex;align-items:center;gap:4px">' + esc(u) + ' <span style="cursor:pointer;opacity:.6" onclick="event.stopPropagation();UNIF_SEL=UNIF_SEL.filter(x=>x!==\'' + esc(u).replace(/'/g, "\\'") + '\');renderUnificar()">✕</span></span>';
      });
      mp += '</div>';
      ...
      mp += '<button class="btn btn-s" onclick="UNIF_SEL=[];renderUnificar()">Cancelar</button>';
      mp += '</div></div>';
...
export function initSeparacao() {
  ...
  try {
    const EXPORTS = { goTab, registrar, previewReq, pegarParaSeparar, entregarReq, abrirFicha, fecharFicha, marcarPronto, marcarProntoLista, voltarSeparacao, printReq, printFicha, cancelarReq, excluirHistoricoReq, renderBuracos, renderUnificar, buildPainel, gerarRelatorio, exportarCSV, handleFile, handleHistFiles, ck, okModal, closeModal, showModal, editEntryYear, editEntryPeriod, removeHistEntry, openEditor, closeEditor, saveEditor, edRemoveItem, edAddItem, edAddCat, clearHistDB, removeDuplicatesAuto, recalcAllDates, exportBackup, importBackup, goToFile, goPage, onModeChange, clearFilters, clearPanFilters, clearYears, selAllYears, clearAllAliases, doUnifMerge, toggleUnifSel, removeAlias, openPrintBuracos, doPrintBuracos, showOrigemUnidade, showOrigemCategoria, renderRelatorio, setPanTipo, loadAllHistDBAndRefresh, PAGE_STATE, debouncedRenderPS, debouncedRenderES, debouncedRenderPE, debouncedRenderHI };
