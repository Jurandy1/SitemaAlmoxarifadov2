...
import { getFirestore, collection, getDocs, getDoc, doc, updateDoc, arrayUnion, addDoc, serverTimestamp, Timestamp, setDoc } from "firebase/firestore";
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        try {
            const roleSnap = await getDoc(doc(COL_USER_ROLES, user.uid));
            const role = roleSnap.exists() ? (roleSnap.data()?.role || '') : '';
            if (role !== 'admin') {
                modalAuth.classList.remove('hidden');
                appContent.classList.add('hidden');
                loginMsg.textContent = "Permissão negada: apenas Admin pode usar o Importador.";
            ...
            modalAuth.classList.remove('hidden');
            appContent.classList.add('hidden');
            loginMsg.textContent = "Erro ao verificar permissões. Tente novamente.";
            loginMsg.classList.remove('hidden');
            return;
        }
...
function renderPreview() {
    ...
    dadosProcessados.forEach((item, index) => {
        ...
        const sugestaoHtml = item.match 
            ? `<div class="text-sm"><div class="font-medium">${escapeHTML(item.match.nome)}</div><div class="text-xs text-gray-500">${escapeHTML(item.matchType || 'desconhecido')}</div></div>`
        ...
        tr.innerHTML = `
            ...
            <td class="p-3 font-mono text-xs text-gray-700">${escapeHTML(item.unidadeSql)}</td>
...
function criarOpcoesUnidades() {
    ...
    return sorted.map(u => `<option value="${escapeHTML(u.id)}">${escapeHTML(u.nome)}</option>`).join('');
}
